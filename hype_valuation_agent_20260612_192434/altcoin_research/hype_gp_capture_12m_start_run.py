#!/usr/bin/env python3
# DEPRECATED — superseded by crypto_valuation_agents_bundle/hype_gp_capture_12m_start_run.py
# Key differences: this file uses 0.034% take-rate and supply/emission scenarios;
# the active model uses 0.026% clean revenue take-rate and velocity-decay scenarios.
...
def derive_hl_volume_market_share(rev_rows, scaled_binance_daily) -> dict:
    ...
    mcp = load_defillama_mcp_metrics()
    ...
    source = "DefiLlama MCP reported derivatives volume" if mcp.get("volume_derivatives_90d_usd") else "DeFiLlama revenue / fixed 0.034% take-rate"
