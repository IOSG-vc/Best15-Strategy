...
def binance_btc_futures_daily():
    ...
    # Monthly ZIPs appear only after month-end, so patch the current month with daily ZIPs.
    import zipfile
    by_date = {}
    ...
    y, m = 2022, 1
    # Completed months only; current month handled by daily files below.
    ...
    # Current month daily patch through yesterday / available files.
    d = now.replace(day=1).date()
    end = now.date() - timedelta(days=1)
    while d <= end:
        url = f"https://data.binance.vision/data/futures/um/daily/klines/BTCUSDT/1d/BTCUSDT-1d-{d:%Y-%m-%d}.zip"
        ingest_zip(url)
        d += timedelta(days=1)
        time.sleep(0.03)
...
def scaled_binance_futures_daily():
    """Daily total Binance Futures proxy: BTCUSDT quote volume scaled by Blockworks annual totals."""
...
def monthly_log_returns_from_monthly(monthly):
    ...
    return np.clip(rets, lo, hi)


def choose_monthly_return_distribution():
    scaled_daily, shares = scaled_binance_futures_daily()
    monthly = monthly_sums(scaled_daily)
    return monthly, monthly_log_returns_from_monthly(monthly), shares
...
def run_model():
    ...
    return {
        ...
        "outputs": {"y3_ttm_gp":pct_dict(y3_ttm_gp),"y3_supply":pct_dict(y3_supply),"multiple_counts":{"10x_peak":int((multiple==10).sum()),"15x_normal":int((multiple==15).sum()),"20x_trough":int((multiple==20).sum())},"y3_token_price":pct_dict(y3_token_price),"discounted_token_price":pct_dict(pv_selected),"price_distribution":{"discounted":pct_dict(pv_selected, CHART_QUANTILES),"undiscounted_y3":pct_dict(y3_token_price, CHART_QUANTILES)},"quantile_components":quantile_components,"prob_impairment_vs_spot":float(np.mean(pv_selected<spot)),"prob_current_spot_justified":float(np.mean(pv_selected>=spot)),"prob_3x_vs_spot":float(np.mean(pv_selected>=3*spot)),"undiscounted_prob_impairment_vs_spot":float(np.mean(y3_token_price<spot)),"undiscounted_prob_current_spot_justified_y3":float(np.mean(y3_token_price>=spot)),"undiscounted_prob_3x_vs_spot":float(np.mean(y3_token_price>=3*spot))},
